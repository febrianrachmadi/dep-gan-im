...
def convert_from_1hot(label, to_float=False):
    N, H, W, C = label.shape
    # print " --> SIZE = " + str((N, H, W, C))
    label_flat = label.reshape((N * H * W, C))
    n_data = len(label_flat)
    # print " --> SIZE reshaped= " + str(label_flat.shape)
    # print " --> LEN = " + str(n_data)
    ...
    # print " --> N-Class-SIZE = " + str(label_n_class.shape)
...
for fold in [1,2,3,4]:
    ...
    my_network.load_weights('./models/' + saved_model_name + str(fold) + '.h5')
    ...
    for data in data_list_flair_1tp:
        if os.path.isfile(data):
            ...
            print "FLR 1tp SHAPE: " + str(brain_flair_1tp.shape)

            # Create output directories for each data
            dirOutData = dirOutput + '/' + str(data_list_name[id])
            ...
            for repeat in range(n_repeat):
                ...
                output_img_pred = my_network.predict([brain_flair_1tp,noise])
            ...
            print "Output SHAPE [1hot]: " + str(output_img_pred.shape)
            output_img_pred_lbl = convert_from_1hot(output_img_pred)
            print "Output SHAPE [lbls]: " + str(output_img_pred_lbl.shape)
            ...
            wmh_mask[output_img_pred_lbl > 0] = 1
            ...
            ## If WMH is GROWING or SHRINKING
            true_pred = 0
            true_prog = 0
            true_regg = 0
            prog = 0
            ...
            print "[PRED, MSE_VOL]: " + str([true_pred, prog, true_prog, regg, true_regg, mse_vol, err_vol])

            ## Spatial Dynamic WMH evolution
            print " ---> WMH Change Mask (Fake)"
            wmh_change_mask_fake = output_img_pred_lbl
            wmh_change_mask_real = np.squeeze(brain_cod_2tp)
            ...
            vol_dsc_all.append(vol_dsc)

            print "DICE metrics #1: " + str([dice_1, dice_2, dice_3, dice_4, avg_all_dice])
            print "DICE metrics #2: " + str([dice_5, dice_6, avg_dice__56])

            output_img = data_prep_save(output_img_pred_lbl)
            nim = nib.Nifti1Image(output_img.astype('int8'), loaded_data_f_1tp.affine)
            nib.save(nim, dirOutData + '/' + data_list_name[id] + '_cls_map.nii.gz')

            N, H, W, C = output_img_pred.shape
            for c in range(C):
                pred_prob = output_img_pred[:,:,:,c]
                pred_prob = pred_prob.reshape((N, H, W, 1))

                output_img = data_prep_save(pred_prob)
                nim = nib.Nifti1Image(output_img.astype('float32'), loaded_data_f_1tp.affine)
                nib.save(nim, dirOutData + '/' + data_list_name[id] + '_prb_map_c'+str(c)+'.nii.gz')
            print "\n"
            
            id += 1

            ## Save all evaluations to .csv file
            numpy_vol_dsc_best_all = np.array(vol_dsc_all)
            f = open(dirOutput + '/vol_dsc_all.csv', 'w')
