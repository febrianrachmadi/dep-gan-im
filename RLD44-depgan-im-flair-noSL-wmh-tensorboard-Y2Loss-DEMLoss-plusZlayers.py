...
filename_save = "depgan_im_flair_noSL_progression_regression_Y2Loss_DEMLoss_plusZlayers_RLD44_08022019"
...
## All losses combined
loss = (-loss_fake) + loss_delta_M1 + loss_delta_M3 + loss_delta_M4 # r4: (-MSE)

training_updates = Adam(lr=lrG, beta_1=0.0, beta_2=0.9).get_updates(netG.trainable_weights,[], loss)
netG_train = K.function([netG_real_input, netD_real_input, noiseZ], [loss, loss_fake, loss_delta_M1, loss_delta_M3, loss_delta_M4], training_updates) # v2
...
for epoch in range(niter):
    ...
    while i < batches:
        ...
        errG,errG_CY2,errG_MSE,errG_VOL,errG_WMH = netG_train([real_data_1tp, real_data_2tp, noise]) # v3
        ...
        print('GEN ERR - [%d/%d][%d/%d][%d] errG: %f errG_CY2: %f errG_MSE: %f errG_VOL: %f errG_WMH: %f'
        % (epoch, niter, i, batches, gen_iterations, errG, errG_CY2, errG_MSE, errG_VOL, errG_WMH))

        ## Save models
        netG.save('./models/netG_'+filename_save+'.h5')
